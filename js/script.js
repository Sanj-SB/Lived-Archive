...
window.submitArtifact = () => {
  // Check if the global state exists before calling
  if (window.currentSubmitterInfo === undefined || window.pendingArtifacts === undefined) {
    console.error("Global state (currentSubmitterInfo or pendingArtifacts) is not defined on window.");
    alert("Submission error: Global state not found.");
    return;
  }
  
  // Call your existing, clean function with the global state
